...
## 1. Khởi Chạy & Key System

### 🔑 Hệ Thống Xác Thực Key (Key System)
R-Client Pro sử dụng hệ thống xác thực key qua **PlatoBoost** để bảo vệ script.

* **Lần đầu tiên chạy**: Nhập key của bạn vào ô nhập liệu và nhấn **VERIFY KEY**. Nhấn **GET KEY** để sao chép link lấy key vào clipboard.
* **Tự động đăng nhập (Key Cache)**: Sau khi xác thực thành công, key sẽ được lưu tự động vào máy. **Những lần chạy tiếp theo, tool sẽ tự động đăng nhập mà không cần nhập lại** — miễn là key vẫn còn hạn.
* **Key hết hạn**: Nếu key không còn hạn, cache sẽ tự động bị xóa và tool sẽ yêu cầu bạn nhập key mới.
...
* **Auto Heal (Hồi Máu Tự Động)**: Khi số pet bị chết đạt ngưỡng cấu hình, nhân vật tự động dịch chuyển đến suối hồi máu gần nhất, chờ hồi đầy rồi quay về bãi tiếp tục farm.
...
* **Tự Động Nhận Biết Ô Ấp (Smart Slots)**: Tool tự động quét trạng thái mở khóa của các ô ấp (từ 1 đến 10) trong game và tự động nạp trứng vào các ô trống đã mở khóa.
...
### 🐾 Pet Manager (Quản Lý Pet Tự Động)
Module quản lý tự động toàn bộ chu kỳ của Pet:

* **Auto Equip Best Pets**: Tự động trang bị các pet có chỉ số cao nhất vào đội hình chiến đấu.
* **Auto Ride Mount**: Tự động cưỡi thú khi nhân vật đang di chuyển để tăng tốc độ.
* **Auto Feed (Level Up)**:
  * Tự động dùng các pet mồi để tăng cấp cho pet đang trang bị.
  * Chọn **độ hiếm (Rarity) của pet mồi** qua dropdown: Common, Uncommon, Rare, Epic, Legendary, Mythical.
* **Auto Enhance**: Tự động sử dụng vàng và nguyên liệu để cường hóa pet đang trang bị.
* **Auto Fuse (Evolve — Ghép Pet Tiến Hóa)**:
  * Tự động ghép các pet trùng lặp trong túi để tiến hóa lên rank cao hơn.
  * Ví dụ: B → A → S → SS theo cơ chế ghép 3 pet cùng loại.
* **Auto Sell**:
  * Tự động bán các pet không trang bị theo phẩm chất bạn chọn.
  * Cấu hình **Batch Size** (số pet bán mỗi lượt, từ 1-50) để tránh quá tải server request.

  > [!WARNING]
  > Luôn kiểm tra kỹ danh sách **Sell Qualities** trước khi bật Auto Sell. Chọn nhầm phẩm chất có thể dẫn đến bán mất pet quý.
...
* **Dọn Dẹp Bộ Nhớ & Logs (Memory & Log Cleaner)**:
  ...
  * **Nút bấm Clear Memory & Logs Now**: Bấm để lập tức chạy dọn rác bộ nhớ sâu. Sau khi dọn dẹp sẽ hiện thông báo chính xác số dung lượng MB RAM đã được giải phóng.
...
2. **Chặn thông báo hệ thống**: Tự động can thiệp vào các bảng thông báo hệ thống của Roblox để lọc bỏ và che giấu tên thật của bạn.
3. **Quét sạch văn bản trên màn hình (UI Sweeper)**: Quét liên tục mỗi 0.1 giây toàn bộ giao diện màn hình. Bất kỳ từ khóa nào trùng với tên thật hoặc DisplayName của bạn sẽ lập tức bị đè và thay thế bằng tên giả, đảm bảo không bị lộ thông tin khi quay màn hình hoặc phát trực tiếp.
