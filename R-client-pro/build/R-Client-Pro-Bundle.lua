-- ====================================================================
-- R-CLIENT PRO BUNDLE (AUTO-BUNDLED ALL MODULES & AUTO EXECUTE)
-- ====================================================================

-- =================================================================
-- KHÓA CHỐNG NHÂN BẢN (GLOBAL LOCK) 
-- =================================================================
if getgenv().R_Client_Pro_Running then 
    warn("⏳ Tool đã hoạt động, chặn thực thi trùng lặp! (Khởi động lại game nếu muốn test bản mới)")
    return 
end
getgenv().R_Client_Pro_Running = true

-- =================================================================
-- KHỞI TẠO ĐƯỜNG DẪN (ĐỒNG BỘ NHÁNH VỚI MAIN.TXT)
-- =================================================================
-- [!] Thay đổi nhánh ở đây cho khớp với main.txt đang test
...
-- =================================================================
-- 1. KỸ THUẬT A: VÒNG LẶP QUEUE ON TELEPORT VĨNH CỬU
-- =================================================================
local executeCode = [[
    if not game:IsLoaded() then game.Loaded:Wait() end
    
    -- Đợi PathTool tải xong hoàn toàn để chống Crash ngầm
    repeat task.wait(0.5) until getrenv()._G and getrenv()._G.PathTool
    
    -- Load lại file auto_execute xuyên cache
    pcall(function()
...
modules['auto_execute.txt'] = function(...)
-- =================================================================
-- KHÓA CHỐNG NHÂN BẢN (GLOBAL LOCK) 
-- =================================================================
if getgenv().R_Client_Pro_Running then 
...
-- =================================================================
-- 2. CHẠY MAIN SCRIPT Ở SERVER HIỆN TẠI (CÓ BẮT LỖI CÚ PHÁP)
-- =================================================================
task.spawn(function()
    ...
    if success then
        ...
        if func then 
            ...
            -- Bắt quả tang ngay nếu main.txt bị lỗi gõ phím
            warn("❌ LỖI CÚ PHÁP TẠI MAIN.TXT:\n" .. tostring(err)) 
        end
    else
        warn("❌ KHÔNG KẾT NỐI ĐƯỢC VỚI GITHUB! Kiểm tra lại Link hoặc Mạng.")
    end
end)

--print("🚀 Đã kích hoạt hệ thống Bám Dính Vĩnh Cửu (Nhánh: " .. branch .. ")!")
...
-- ==========================================
-- KIỂM TRA NHÂN VẬT CÓ ĐANG TRONG PHÓ BẢN ĐẶC BIỆT KÍNH (RIFT / ABYSS...)
-- ==========================================
function Utils.IsInSpecialArea()
    local areaId = Utils.GetCurrentAreaId()
    return areaId == "Rift" or areaId == "Abyss"
end
...
-- ==========================================
-- HÀM GOM PET VỀ VỊ TRÍ NHÂN VẬT (TELEPORT PETS TO PLAYER)
-- ==========================================
function Utils.GatherPetsToPlayer()
    pcall(function()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
...
-- 1. Lên / Xuống thú cưỡi (Kế hợp Remote Server + UI Click tức thì)
function Utils.ToggleMount(state)
    local isRiding = LocalPlayer:GetAttribute("RidePetId") ~= nil
    if (state and isRiding) or (not state and not isRiding) then
        return
    end

    -- 1. Gửi Remote kích hoạt từ Server
    local remote = GetDataPullFunc()
    if remote then
        pcall(function() remote:InvokeServer("PetSwitchRideStatusChannel", state) end)
    end

    -- 2. Nhấp nút UI Ride trên QuickBar để tạo hiệu ứng nhấp màn hình chuẩn
    pcall(function()
        local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
        local mainGui = playerGui and playerGui:FindFirstChild("MainGui")
        local screenGui = mainGui and mainGui:FindFirstChild("ScreenGui")
        local quickBar = screenGui and screenGui:FindFirstChild("MainQuickBarView")
        local buttonList = quickBar and quickBar:FindFirstChild("ButtonList")
        local rideButton = buttonList and buttonList:FindFirstChild("Ride")

        if rideButton and rideButton.Visible then
            if Utils and type(Utils.ClickButtonExact) == "function" then
                Utils.ClickButtonExact(rideButton, "Ride")
            end
        end
    end)
end
...
function Utils.ResolveMonsterName(monsterObj)
    ...
    local uidNum = uidStr and tonumber(uidStr) or nil
    if uidNum and monsterNameCache[uidNum] then
    ...
    if uidStr then
        ...
        if cleanName ~= "" then
            if uidNum then monsterNameCache[uidNum] = cleanName end
            return cleanName
        end
    end

    if uidNum then monsterNameCache[uidNum] = originalName end
    return originalName
end
                ...
                for _, monster in pairs(monstersFolder:GetChildren()) do
                    if monster:IsA("Model") or monster:IsA("BasePart") then
                        ...
                        if scanMode == "Catch" then
                            local catchTaken = monster:GetAttribute("CatchTakenPlayerId") or root:GetAttribute("CatchTakenPlayerId")
                            if catchTaken and catchTaken == playerId then isValid = true end
                        else
                            -- Chế độ Attack: Bỏ qua nếu quái đang chờ bắt (đã bị hạ gục bởi người khác)
                            local catchTaken = monster:GetAttribute("CatchTakenPlayerId") or root:GetAttribute("CatchTakenPlayerId")
                            if not catchTaken then
                                if Utils.IsMonsterAlive(monster) then 
                                    isValid = true 
                                end
                            end
                        end
...
function Utils.IsMonsterAlive(monsterObj)
    ...
    if monsterObj:GetAttribute("CatchTakenPlayerId") or (root and root:GetAttribute("CatchTakenPlayerId")) then
        return false
    end
    ...
    if hpVal and hpVal:IsA("StringValue") then
        ...
        if #parts == 2 then
            ...
            -- Thử parse thập phân thường
            hpNum = tonumber((string.gsub(rawStr, ",", ".")))
        end

        return (hpNum and hpNum > 0) or false
    end
    
    -- Mặc định trả về true nếu không tìm thấy thanh máu (boss phụ hoặc quái map đặc biệt chưa kịp tải)
    -- nhưng BillboardGui vẫn hiển thị
    return true
end
...
local Translations = {
    ...
    -- Pet Manager keys
    ["pet_manager"] = { en = "Pet Manager", vi = "Quản Lý Pet" },
    ["sec_pet_lock"] = { en = "🔒 PET LOCK & UNLOCK MANAGEMENT", vi = "🔒 QUẢN LÝ KHÓA / MỞ KHÓA PET" },
    ["batch_lock_filter"] = { en = "Select Pet Criteria (Batch Filter)", vi = "Tiêu Chí Chọn Pet (Batch Lock Filter)" },
    ["batch_lock_filter_info"] = { en = "Select one or multiple criteria to lock or unlock pets in batch.", vi = "Tích chọn 1 hoặc nhiều loại Pet bạn muốn thao tác Khóa / Mở Khóa." },
    ["criteria_rank_a"] = { en = "Rank A & Above (Grade 4+)", vi = "Rank A Trở Lên (Grade 4+)" },
    ["criteria_mutation"] = { en = "Mutated Pets (Shiny/Huge/Bloodlit...)", vi = "Pet Có Dị Biến (Shiny/Huge...)" },
    ["criteria_high_trait"] = { en = "High Quality Traits (Q: 5-6)", vi = "Pet Có Trait Cao Cấp (Q: 5-6)" },
    ["criteria_all_trait"] = { en = "All Trait / Talent Pets", vi = "Tất Cả Trait / Thiên Phú" },
    ["criteria_all_pets"] = { en = "All Pets in Inventory", vi = "Tất Cả Pet Trong Túi" },
    ["btn_batch_lock"] = { en = "🔒 LOCK PETS BY SELECTED CRITERIA", vi = "🔒 KHÓA PET THEO TIÊU CHÍ ĐÃ CHỌN" },
    ["btn_batch_lock_info"] = { en = "Locks all pets that match any of the selected criteria above.", vi = "Thực hiện khóa tất cả Pet trùng khớp với các mục bạn đã tích ở trên." },
    ["btn_batch_unlock"] = { en = "🔓 UNLOCK PETS BY SELECTED CRITERIA", vi = "🔓 MỞ KHÓA PET THEO TIÊU CHÍ ĐÃ CHỌN" },
    ["btn_batch_unlock_info"] = { en = "Unlocks all pets that match any of the selected criteria above.", vi = "Thực hiện mở khóa tất cả Pet trùng khớp với các mục bạn đã tích ở trên." },
    ["sec_pet_fuse"] = { en = "🧬 SMART PET FUSE & EVOLVE", vi = "🧬 DUNG HỢP PET THÔNG MINH" },
    ["max_fuse_rank"] = { en = "Max Fuse Rank Limit", vi = "Phẩm Chất Dung Hợp Tối Đa" },
    ["max_fuse_rank_info"] = { en = "Set the maximum pet rank allowed to be consumed as fusion ingredients.", vi = "Chọn Rank Pet tối đa được phép gom làm phôi dung hợp." },
    ["fuse_option_rank_b"] = { en = "Rank B (Grade 3 - Safe Default)", vi = "Rank B (Grade 3 - Mặc định an toàn)" },
    ...
    ["skip_trait_pets_info"] = { en = "Never consumes pets with Traits/Talents (Impair, Fortify, Hasten...) as fusion ingredients.", vi = "Tuyệt đối KHÔNG dùng Pet sở hữu Trait/Thiên phú (Impair, Fortify, Hasten...) làm phôi dung hợp." },
    ["skip_locked_pets"] = { en = "Skip Locked Pets", vi = "Bỏ Qua Pet Đang Khóa" },
    ["skip_locked_pets_info"] = { en = "Does not use locked pets as fusion ingredients.", vi = "Không sử dụng Pet đang bị khóa làm phôi dung hợp." },
    ["skip_equipped_pets"] = { en = "Skip Equipped / Team Pets", vi = "Bỏ Qua Pet Đang Ra Trận" },
    ["skip_equipped_pets_info"] = { en = "Does not use pets currently equipped or in active battle team.", vi = "Không sử dụng Pet đang ra trận/trang bị làm phôi." },
    ["auto_fuse_bag_full"] = { en = "🎒 Auto Fuse Only When Bag Nearly Full", vi = "🎒 Chỉ Auto Fuse Khi Túi Sắp Đầy" },
    ["auto_fuse_bag_full_info"] = { en = "Triggers auto fusion only when free inventory slots drop below 5.", vi = "Chỉ tự động ghép Pet khi số ô trống trong túi còn ít hơn 5 chỗ." },
    ["auto_fuse_pets"] = { en = "Auto Fuse Pets", vi = "Auto Dung Hợp Pet (Auto Fuse)" },
    ...
    ["sec_timed_rejoin"] = { en = "TIMED SERVER REJOIN / HOP", vi = "TỰ ĐỘNG ĐỔI SERVER THEO THỜI GIAN" },
    ["auto_rejoin_timer"] = { en = "Timed Server Hop", vi = "Tự Động Đổi Server" },
    ["auto_rejoin_timer_info"] = { en = "Automatically switches to a new server after playing for a specific amount of time to reduce lag.", vi = "Tự động chuyển sang server mới sau một khoảng thời gian chơi nhất định để tránh giật lag." },
    ["rejoin_interval"] = { en = "Rejoin Interval (Minutes)", vi = "Thời Gian Nhảy Server (Phút)" },
    ["rejoin_interval_info"] = { en = "Set the amount of minutes to wait before hopping to another server.", vi = "Thiết lập số phút chơi trước khi tự động nhảy sang server mới." },
    ["rejoin_timer_label"] = { en = "Next Hop In: ", vi = "Nhảy server tiếp theo sau: " },
    ...
    ["dg_claim_reward"] = { en = "🎁 Auto Claim Reward", vi = "🎁 Auto Claim Reward" },
    ["dg_claim_reward_info"] = { en = "Loads and executes claim_reward.txt script for dungeon reward / buff picking.", vi = "Tải và chạy script claim_reward.txt (Giao diện Auto Chọn Buff & Claim quà Dungeon)." },
    ...
    ["sec_dg_lobby"] = { en = "IN LOBBY", vi = "NGOÀI SẢNH " },
    ...
    ["farm_guide_content"] = { en = "• TP Farm: Teleport directly to monsters.\n• Auto Catch: Capture monsters when low HP.\n• Smart Return: Teleport back to farm center after healing/dying.\n• Auto Egg Pickup: Teleports and collects egg drops in AreaPickUp across islands.", vi = "• TP Farm: Dịch chuyển áp sát quái để diệt nhanh hơn.\n• Auto Catch: Tự động ném bóng thu phục quái yếu máu.\n• Smart Return: Tự động bay về tâm bãi sau khi hồi phục/hồi sinh.\n• Auto Nhặt Trứng: Tự động bay qua các đảo đã chọn và thu thập toàn bộ trứng trong AreaPickUp." },

    -- Auto Egg Pickup keys
    ["sec_egg_pickup"] = { en = "🥚 AUTO EGG PICKUP (AREAPICKUP)", vi = "🥚 TỰ ĐỘNG NHẶT TRỨNG (AREAPICKUP)" },
    ["egg_pickup_master_toggle"] = { en = "Auto Egg Pickup", vi = "Auto Nhặt Trứng" },
    ["egg_pickup_master_toggle_info"] = { en = "Automatically cycles through selected islands and teleports to collect all egg drops in AreaPickUp.", vi = "Tự động bay qua các đảo đã chọn và thu thập tất cả trứng trong AreaPickUp trên bản đồ." },
    ["egg_pickup_status_off"] = { en = "STATUS: AUTO EGG PICKUP OFF", vi = "TRẠNG THÁI: AUTO NHẶT TRỨNG ĐANG TẮT" },
    ["egg_pickup_status_prep"] = { en = "STATUS: PREPARING SCAN...", vi = "TRẠNG THÁI: ĐANG CHUẨN BỊ QUÉT..." },
    ["egg_pickup_status_map"] = { en = "STATUS: Island %s (%d eggs left)...", vi = "TRẠNG THÁI: Đảo %s (Còn %d quả)..." },
    ["egg_pickup_status_done_cycle"] = { en = "STATUS: Cycle completed! Resting...", vi = "TRẠNG THÁI: Hoàn thành 1 vòng! Đang nghỉ..." },
    ["egg_pickup_count_label"] = { en = "Eggs Collected: ", vi = "Trứng đã nhặt: " },
    ...
    -- Auto Event Skyheart keys
    ["auto_return_skyheart"] = { en = "Auto Teleport to Skyheart on Event", vi = "Tự Động Về Skyheart Khi Event" },
    ["auto_return_skyheart_info"] = { en = "Automatically detects when an event starts and teleports directly to Skyheart Island to participate.", vi = "Tự động nhận diện khi sự kiện bắt đầu và dịch chuyển tức thời về đảo Skyheart để tham gia." },
    ["return_farm_after_event"] = { en = "Return to Farm Spot After Event", vi = "Quay Về Bãi Farm Sau Khi Hết Event" },
    ["return_farm_after_event_info"] = { en = "Automatically teleports back to your previous farming spot after the Skyheart event finishes.", vi = "Tự động bay về vị trí bãi farm trước đó sau khi sự kiện trên đảo Skyheart kết thúc." },
    ["event_status_teleporting_skyheart"] = { en = "STATUS: EVENT DETECTED! TELEPORTING TO SKYHEART...", vi = "TRẠNG THÁI: PHÁT HIỆN EVENT! ĐANG VỀ SKYHEART..." },
    ["event_status_returning_farm"] = { en = "STATUS: EVENT FINISHED! RETURNING TO FARM SPOT...", vi = "TRẠNG THÁI: HẾT EVENT! ĐANG VỀ LẠI BÃI FARM..." },
    ["event_notify_title"] = { en = "🎉 Auto Event", vi = "🎉 Auto Event" },
    ["event_notify_skyheart"] = { en = "Event started! Teleporting to Skyheart Island...", vi = "Sự kiện đã bắt đầu! Đang dịch chuyển về đảo Skyheart..." },
    ["event_notify_finished"] = { en = "Event completed! Returning to previous farming spot...", vi = "Sự kiện đã kết thúc! Đang quay về bãi farm trước đó..." },
    ["event_stop_on_limit"] = { en = "Stop On Limit Reached (100/100)", vi = "Dừng Khi Đạt Giới Hạn Quà (100/100)" },
    ["event_stop_on_limit_info"] = { en = "Automatically detects when the event has reached max reward limit (e.g. 100/100) to stop participating and resume auto-farming.", vi = "Tự động phát hiện khi sự kiện đã nhận đủ quà tối đa (100/100) để dừng tham gia và quay về tiếp tục auto farm." },
    ["event_status_limit_reached"] = { en = "STATUS: EVENT LIMIT REACHED (%d/%d)! PAUSED...", vi = "TRẠNG THÁI: ĐÃ ĐẠT GIỚI HẠN (%d/%d)! ĐANG DỪNG..." },
    ["event_limit_hud"] = { en = "⚠️ Event reward limit reached: %d/%d. Auto-stopped to save time.", vi = "⚠️ Đã đạt giới hạn nhận quà: %d/%d. Tự động dừng để tiết kiệm thời gian." }
}
...
return function(Window, Utils)
    ...
    local wbCycleCompleted = false -- Cờ phân biệt "chưa bắt đầu" vs "vừa kết thúc vòng"
    ...
    Utils.CreateFilteredAreaMultiSelect(BossTab, {
        ...
        callback = function(selectedIds)
            wbSettings.selectedWorlds = selectedIds
            if wbSettings.enabled and not wbHuntState.isActive and not wbCycleCompleted and #wbSettings.selectedWorlds > 0 then
                print("🌍 [WORLD BOSS] Cập nhật danh sách đảo. Tự động kích hoạt luồng săn Boss...")
        ...
        while task.wait(0.5) do
            pcall(function()
                ...
                if wbSettings.enabled and not wbHuntState.isActive then
                    ...
                    if wbWorldId then
                        -- Kiểm tra xem Đảo phát ra tín hiệu có nằm trong danh sách Đảo người chơi đã chọn không
                        local isWorldSelected = false
                        for _, aId in ipairs(wbSettings.selectedWorlds) do
                            if tonumber(aId) == tonumber(wbWorldId) then
                                isWorldSelected = true
                                break
                            end
                        end

                        if isWorldSelected then
                            if os.time() - wbLastWaveAlertTime >= WB_WAVE_COOLDOWN_SECS then
                                wbLastWaveAlertTime = os.time()
                                SaveWbWaveEndTime()
                                
                                print("⚔️ [SÓNG WORLD BOSS] Kích hoạt càn quét từ Đảo chọn " .. tostring(wbWorldId) .. " (Tín hiệu: " .. currentText .. ")")
                                StartWbHuntFromFirstWorld()
    ...
    task.spawn(function()
        ...
        while task.wait(0.2) do
            -- [Tự động kích hoạt khi cấu hình dropdown nạp trễ hơn toggle ở startup]
            if wbSettings.enabled and not wbHuntState.isActive and not wbCycleCompleted and #wbSettings.selectedWorlds > 0 then
                print("🌍 [WORLD BOSS] Tự động khởi chạy luồng săn Boss khi danh sách đảo sẵn sàng.")
                StartWbHuntFromFirstWorld()
                wbStatusLabel:Set(Utils.t("boss_req_lock"))
            end
                ...
                if not wbHuntState.hasTeleported then
                    ...
                    if wbHuntState.scanAttempts <= WB_SCAN_TIMEOUT_TICKS then
                        ...
                        if nextIndex > #wbSettings.selectedWorlds then
                            wbHuntState.isActive = false
                            wbCycleCompleted     = true -- Đánh dấu đã xong 1 vòng, ngăn auto-restart ngay
                            ...
                            if wbSettings.autoHop and isWaveActive then
                                ...
                                TryReleaseLock()
                                pcall(function() Utils.HopServer("Săn World Boss") end)
                            else
...
return function(Window, Utils)
    ...
    local autoRejoinTimerEnabled = false -- Biến bật tắt Rejoin theo thời gian
    local rejoinIntervalMinutes = 30 -- Thời gian hẹn giờ Rejoin (phút)
    ...
    pcall(function()
        if isfile and isfile(configFileName) then
            ...
            if parsedData then
                ...
                autoRejoinTimerEnabled = parsedData.autoRejoinTimerEnabled or false
                rejoinIntervalMinutes = parsedData.rejoinIntervalMinutes or 30
            end
        end
    end)
        ...
        pcall(function()
            if writefile then
                local dataToSave = {
                    ...
                    autoRejoinTimerEnabled = autoRejoinTimerEnabled,
                    rejoinIntervalMinutes = rejoinIntervalMinutes
                }
    ...
    ServerTab:CreateSection(" " .. Utils.t("sec_timed_rejoin") .. " ")
    local RejoinTimerLabel = ServerTab:CreateLabel(Utils.t("rejoin_timer_label") .. "--:--")
    
    ServerTab:CreateToggle({
        Name = Utils.t("auto_rejoin_timer"),
        Info = Utils.t("auto_rejoin_timer_info"),
        CurrentValue = autoRejoinTimerEnabled,
        Callback = function(Value)
            autoRejoinTimerEnabled = Value
            SaveState()
        end
    })
    
    ServerTab:CreateSlider({
        Name = Utils.t("rejoin_interval"),
        Info = Utils.t("rejoin_interval_info"),
        Range = {5, 120},
        Increment = 5,
        CurrentValue = rejoinIntervalMinutes,
        Callback = function(Value)
            rejoinIntervalMinutes = Value
            SaveState()
        end
    })
    ...
    -- LUỒNG ĐỔI SERVER THEO HẸN GIỜ (TIMED SERVER HOP)
    task.spawn(function()
        while task.wait(1) do
            if autoRejoinTimerEnabled then
                local elapsed = os.time() - sessionStartTime
                local targetSeconds = rejoinIntervalMinutes * 60
                local timeLeft = targetSeconds - elapsed
                
                if timeLeft > 0 then
                    local minutes = math.floor(timeLeft / 60)
                    local seconds = timeLeft % 60
                    RejoinTimerLabel:Set(Utils.t("rejoin_timer_label") .. string.format("%02d:%02d", minutes, seconds))
                else
                    RejoinTimerLabel:Set(Utils.t("rejoin_timer_label") .. "HOPPING...")
                    
                    -- Kiểm tra xem có thể nhảy Server ngay không (tránh ngắt quãng Boss/Dungeon)
                    local controller = _G.SystemController
                    local canHop = true
                    if controller and controller.CurrentLock then
                        local lock = controller.CurrentLock
                    ...
                    if canHop then
                        print("[ServerManager] ⏰ Hết thời gian phiên chơi (" .. tostring(rejoinIntervalMinutes) .. " phút). Đang chuyển server...")
                        pcall(function() Utils.HopServer("Hẹn giờ nhảy Server") end)
                        task.wait(10)
                    else
                        RejoinTimerLabel:Set(Utils.t("rejoin_timer_label") .. "WAITING (" .. tostring(controller.CurrentLock) .. ")")
                    end
                end
            else
                RejoinTimerLabel:Set(Utils.t("rejoin_timer_label") .. "DISABLED")
            end
        end
    end)
    
    print("✅ Server Manager 4.3 (Timed Server Hop & Persistence Loaded!)")
end
...
return function(Window, Utils, WebhookModule)
    ...
    DungeonTab:CreateButton({
        Name = Utils.t("dg_claim_reward"),
        Info = Utils.t("dg_claim_reward_info"),
        Callback = function()
            task.spawn(function()
                local rawUrl = "https://raw.githubusercontent.com/chaocauminhlason/scripts-linh-tinh/refs/heads/test/pre-config-manager/dungoen-script/claim_reward.txt?t=" .. os.time()
                    ...
                    if func then
                        ...
                        pcall(function()
                            game:GetService("StarterGui"):SetCore("SendNotification", {
                                Title = "Auto Dungeon",
                                Text = "Đã tải thành công GUI Claim Reward!",
                                Duration = 3
                            })
                        end)
                    else
                        warn("Lỗi cú pháp claim_reward.txt: " .. tostring(err))
                    end
                else
                    warn("Không thể tải file claim_reward.txt từ GitHub.")
                end
            end)
        end
    })
    ...
    DungeonTab:CreateDropdown({
        ...
        Options = {"1 - Normal", "2 - Hard", "3 - Nightmare", "4 - Inferno", "5 - Cataclysm" }, 
        ...
        Callback = function(Option)
            ...
            elseif Option[1] == "4 - Inferno" then targetDifficulty = 4 
            elseif Option[1] == "5 - Cataclysm" then targetDifficulty = 5 
            end
        end,
    })
        ...
        while task.wait(0.5) do
            ...
            if isInDungeon then
                ...
                if shouldCheckCompletion and not autoExitTriggered then
                    ...
                    if isSettled then
                        isFinished = true
                    elseif currentStage and maxStage and tonumber(currentStage) == tonumber(maxStage) then
                    ...
                    elseif not (currentStage and maxStage and tonumber(currentStage) == tonumber(maxStage)) then 
                        stage20ClockStarted = false
                        autoExitTriggered = false 
                        waitingForHost = false
                        hostMissingTime = nil 
...
return function(Window, Utils)
        ...
        if env and env.CfgEgg and env.CfgEgg.Tmpls then
            ...
            for id, info in pairs(env.CfgEgg.Tmpls) do
                ...
                if eId then
                    ...
                    table.insert(eggList, { str = displayStr, id = eId })
                end
            end

            -- Sắp xếp theo ID trứng tăng dần
            table.sort(eggList, function(a, b) return a.id < b.id end)
    ...
    -- Khởi tạo hoặc khôi phục danh sách chọn từ phiên làm việc trước (khi Hop Server)
    local defaultSelectedEggStrings = {}
    hatchSettings.eggIdsToHatch = {}

    local savedEggIds = getgenv().R_Client_SavedHatchEggIds

    if savedEggIds and type(savedEggIds) == "table" and #savedEggIds > 0 then
        -- KHÔI PHỤC CHÍNH XÁC CÁC LOẠI TRỨNG NGƯỜI DÙNG ĐÃ TÍCH CHỌN TRƯỚC KHI HOP SERVER
        local savedSet = {}
        for _, id in ipairs(savedEggIds) do savedSet[id] = true end

        for str, eId in pairs(eggIdMap) do
            if savedSet[eId] then
                table.insert(defaultSelectedEggStrings, str)
                table.insert(hatchSettings.eggIdsToHatch, eId)
            end
        end
    else
        -- Lần đầu tiên bật tool: Mặc định CHỈ chọn 1 loại trứng đầu tiên để người dùng tự chọn thêm dễ dàng
        if #eggOptions > 0 and eggIdMap[eggOptions[1]] then
            table.insert(defaultSelectedEggStrings, eggOptions[1])
            table.insert(hatchSettings.eggIdsToHatch, eggIdMap[eggOptions[1]])
        end
    end

    if #defaultSelectedEggStrings == 0 and #eggOptions > 0 then
        table.insert(defaultSelectedEggStrings, eggOptions[1])
        if eggIdMap[eggOptions[1]] then
            table.insert(hatchSettings.eggIdsToHatch, eggIdMap[eggOptions[1]])
        end
    end
    ...
    local EggDropdown = EggTab:CreateDropdown({
        ...
        Options = eggOptions,
        CurrentOption = defaultSelectedEggStrings,
        ...
        Callback = function(Options)
            ...
            elseif type(Options) == "string" then
                table.insert(selectedList, Options)
            end

            local newIds = {}
            for _, selectedString in ipairs(selectedList) do
                local eId = eggIdMap[selectedString]
                if not eId then
                    eId = tonumber(string.match(selectedString, "%[ID:%s*(%d+)%]"))
                end
                if eId then
                    table.insert(newIds, eId)
                end
            end
            if #newIds > 0 then
                hatchSettings.eggIdsToHatch = newIds
                hatchCycleIndex = 1 -- Reset chỉ số luân phiên ngay lập tức khi chọn trứng mới
                -- LƯU LẠI VÀO GLOBAL ENVIRONMENT ĐỂ KHÔI PHỤC CHÍNH XÁC KHI HOP SERVER
                getgenv().R_Client_SavedHatchEggIds = newIds
            end
        end,
    })
    ...
    task.spawn(function()
        while task.wait(0.5) do
            ...
            -- Đếm và lọc các loại trứng ĐÃ CHỌN trong túi
                ...
                if #hatchSettings.eggIdsToHatch > 0 then
                    for _, id in ipairs(hatchSettings.eggIdsToHatch) do
                        ...
                        pcall(function()
                            ...
                            set_thread_id(2)
                            if type(clientEgg.GetEggAmount) == "function" then
                                amt = clientEgg:GetEggAmount(id) or clientEgg:GetEggAmount(tostring(id)) or clientEgg:GetEggAmount(tonumber(id)) or 0
                            else
                                amt = 1
                            end
                            set_thread_id(oldId)
                    ...
                    -- Nếu không kiểm tra được số lượng (amt <= 0 cho tất cả), giữ nguyên danh sách các trứng người dùng ĐÃ CHỌN
                    -- tuyệt đối KHÔNG tự động nhảy sang trứng ngẫu nhiên khác!
...
return function(Window, Utils)
    ...
    local function ExecuteExitRift(controller)
        ...
        -- Nhấp nút thoát phó bản vật lý BgLeave (nếu có trên màn hình)
        pcall(function()
            local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
            local mainGui = playerGui and playerGui:FindFirstChild("MainGui")
            local screenGui = mainGui and mainGui:FindFirstChild("ScreenGui")
            if screenGui then
                local arenaRightView = screenGui:FindFirstChild("ArenaMainRightView") or screenGui:FindFirstChild("AbyssMainRightView")
                local buttonList = arenaRightView and arenaRightView:FindFirstChild("ButtonList")
                local bgLeave = buttonList and buttonList:FindFirstChild("BgLeave")
                
                if bgLeave and bgLeave.Visible then
                    if Utils and type(Utils.ClickButtonExact) == "function" then
                        Utils.ClickButtonExact(bgLeave, "Thoát Rift (BgLeave)")
                    end
                end
            end
        end)
            ...
            if riftState == "SCANNING" then
                ...
                if cycleState.isResting then
                    if riftSettings.autoHop then
                        print("[AutoRift] 🔄 Hoàn tất 1 vòng quét Rift -> Đang thực hiện Hop Server...")
                        CycleStatus:Set(Utils.t("rift_cycle_resting") .. " (Hop Server...)")
...
return function(Window, Utils)
    ...
    local SkyheartCenterPos = Vector3.new(507, 3626, -256)
    local eventSessionState = {
        isEventActive = false,
        savedFarmCFrame = nil,
        savedAreaId = nil,
        lastEventSeenTime = 0
    ...
    EventTab:CreateToggle({
        Name = Utils.t("auto_return_skyheart"),
        Info = Utils.t("auto_return_skyheart_info"),
        CurrentValue = true,
        Flag = "AutoReturnSkyheartOnEvent",
        Callback = function(Value)
            eventSettings.autoReturnSkyheart = Value
        end
    })

    EventTab:CreateToggle({
        Name = Utils.t("return_farm_after_event"),
        Info = Utils.t("return_farm_after_event_info"),
        CurrentValue = true,
        Flag = "ReturnFarmAfterEventFinish",
        Callback = function(Value)
            eventSettings.returnAfterEvent = Value
        end
    })

    EventTab:CreateToggle({
        Name = Utils.t("event_stop_on_limit"),
        Info = Utils.t("event_stop_on_limit_info"),
        CurrentValue = true,
        Flag = "EventStopOnLimitReached",
        Callback = function(Value)
            eventSettings.stopOnLimit = Value
        end
    })
    ...
    local function IsPlayerOnSkyheart(hrp)
        if not hrp then return false end
        local centerArea = Workspace:FindFirstChild("Area") and Workspace.Area:FindFirstChild("center")
        if centerArea then
            local region = centerArea:FindFirstChild("AreaRegion") or centerArea:FindFirstChild("SpawnLocation")
            if region and region:IsA("BasePart") then
                return (hrp.Position - region.Position).Magnitude < 800 or hrp.Position.Y > 3000
            end
        end
        return hrp.Position.Y > 3000 or (hrp.Position - SkyheartCenterPos).Magnitude < 800
    end

    local function HasActiveEvent()
        local obbyFolder = Workspace:FindFirstChild("ObbyEventFolder")
        if obbyFolder then
            local qaFolder = obbyFolder:FindFirstChild("QAFolder")
            if qaFolder and #qaFolder:GetChildren() > 0 then return true end

            local newSelectFolder = obbyFolder:FindFirstChild("NewSelectFolder")
            if newSelectFolder and #newSelectFolder:GetChildren() > 0 then return true end

            for _, child in ipairs(obbyFolder:GetChildren()) do
                if string.find(child.Name, "MemePetFind") then
                    local memeFolder = child:FindFirstChild("MemePetFindRuntimeZones")
                    if memeFolder and #memeFolder:GetChildren() > 0 then return true end
                end
            end

            local robbery = obbyFolder:FindFirstChild("Robbery")
            if robbery and #robbery:GetChildren() > 0 then return true end

            local snowman = obbyFolder:FindFirstChild("MakeSnowmanFolder")
            if snowman and #snowman:GetChildren() > 0 then return true end

            local trick = obbyFolder:FindFirstChild("TrickOrTreatFolder")
            if trick and #trick:GetChildren() > 0 then return true end
        end
    ...
    local function CheckEventLimitReached()
        local pgui = LocalPlayer:FindFirstChild("PlayerGui")
        local dialogView = pgui and pgui:FindFirstChild("MainGui") and pgui.MainGui:FindFirstChild("ScreenGui") and pgui.MainGui.ScreenGui:FindFirstChild("DialogView")
        if dialogView then
            for _, desc in ipairs(dialogView:GetDescendants()) do
                if desc:IsA("TextLabel") and desc.Text ~= "" then
                    local current, maxVal = string.match(desc.Text, "%((%d+)%s*/%s*(%d+)%)")
                    if current and maxVal then
                        local cNum = tonumber(current)
                        local mNum = tonumber(maxVal)
                        if cNum and mNum and mNum > 0 and cNum >= mNum then
                            return true, cNum, mNum, desc.Text
                        end
                    end
                end
            end
    ...
    -- Core Loop
    local lastSelectedMemePart = nil
    task.spawn(function()
        while task.wait(0.5) do
            ...
            -- KIỂM TRA GIỚI HẠN SỰ KIỆN (100/100)
            if eventSettings.stopOnLimit then
                local limitReached, currentVal, maxVal = CheckEventLimitReached()
                if limitReached then
                    EventStatus:Set(string.format(Utils.t("event_status_limit_reached"), currentVal, maxVal))
                    LiveHud:Set({
                        Title = "📡 SỰ KIỆN ĐÃ ĐẠT GIỚI HẠN",
                        Content = string.format(Utils.t("event_limit_hud"), currentVal, maxVal)
                    })
                    
                    -- Nếu đang kích hoạt cờ sự kiện -> Nhả cờ và quay về bãi farm cũ
                    if eventSessionState.isEventActive then
                        print("[AutoEvent] Đã đạt giới hạn sự kiện: " .. tostring(currentVal) .. "/" .. tostring(maxVal) .. ". Tự động dừng tham gia.")
                        _G.IsEventActive = false
                        eventSessionState.isEventActive = false
                        
                        if eventSettings.returnAfterEvent and eventSessionState.savedFarmCFrame then
                            EventStatus:Set(Utils.t("event_status_returning_farm"))
                            pcall(function()
                                if Utils and type(Utils.SendNotify) == "function" then
                                    Utils.SendNotify(Utils.t("event_notify_title"), Utils.t("event_notify_finished"))
                                end
                            end)
                            
                            if eventSessionState.savedAreaId and Utils and type(Utils.GetCurrentAreaId) == "function" and Utils.GetCurrentAreaId() ~= eventSessionState.savedAreaId and type(eventSessionState.savedAreaId) == "number" then
                                pcall(function() Utils.TeleportToArea(eventSessionState.savedAreaId) end)
                                task.wait(4)
                            end
                            
                            local charHrp = getHrp()
                            if charHrp then
                                charHrp.CFrame = eventSessionState.savedFarmCFrame
                            end
                            
                            eventSessionState.savedFarmCFrame = nil
                            eventSessionState.savedAreaId = nil
                            task.wait(1)
                        end
                    end
            ...
            -- KIỂM TRA NHẬN DIỆN SỰ KIỆN ĐANG DIỄN RA
            local isEventRunning = HasActiveEvent()
            
            if isEventRunning then
                eventSessionState.lastEventSeenTime = os.time()
                
                -- Nếu phát hiện sự kiện và chưa ở đảo Skyheart -> Tự động dịch chuyển về Skyheart
                if eventSettings.autoReturnSkyheart and not IsPlayerOnSkyheart(hrp) then
                    -- Lưu vị trí bãi farm trước khi bay về Skyheart (chỉ lưu 1 lần)
                    if not eventSessionState.savedFarmCFrame then
                        eventSessionState.savedFarmCFrame = hrp.CFrame
                        eventSessionState.savedAreaId = Utils and type(Utils.GetCurrentAreaId) == "function" and Utils.GetCurrentAreaId() or nil
                    end
                    
                    _G.IsEventActive = true
                    eventSessionState.isEventActive = true
                    EventStatus:Set(Utils.t("event_status_teleporting_skyheart"))
                    
                    pcall(function()
                        if Utils and type(Utils.SendNotify) == "function" then
                            Utils.SendNotify(Utils.t("event_notify_title"), Utils.t("event_notify_skyheart"))
                        end
                    end)
                    
                    -- Dịch chuyển an toàn về đảo Skyheart
                    Utils.SafeTeleport(CFrame.new(SkyheartCenterPos), 5)
                    task.wait(2)
                    continue
                end
                ...
                -- Sự kiện đã kết thúc (hoặc chưa có sự kiện)
                if eventSessionState.isEventActive then
                    -- Kiểm tra xem đã hết event thực sự chưa (buffer 3 giây để tránh giật lag)
                    if os.time() - eventSessionState.lastEventSeenTime >= 3 then
                        print("[AutoEvent] Sự kiện trên Skyheart đã kết thúc!")
                        _G.IsEventActive = false
                        eventSessionState.isEventActive = false
                        
                        -- Tự động quay về bãi farm cũ nếu bật cài đặt
                        if eventSettings.returnAfterEvent and eventSessionState.savedFarmCFrame then
                            EventStatus:Set(Utils.t("event_status_returning_farm"))
                            pcall(function()
                                if Utils and type(Utils.SendNotify) == "function" then
            ...
            if obbyFolder then
                ...
                if qaFolder and #qaFolder:GetChildren() > 0 then
                    local successQa, CfgQA = pcall(function() return env and env.CfgQA end)
                    CfgQA = successQa and CfgQA or nil
                    local activeKey = nil
                    ...
                    -- Fallback: Nếu không có CfgQA hoặc không tìm thấy câu trả lời đúng, chọn theo số đông
                ...
                if not activeObbyPart then
                    ...
                    if newSelectFolder and #newSelectFolder:GetChildren() > 0 then
                        local successNs, CfgNewSelect = pcall(function() return env and env.CfgNewSelect end)
                        CfgNewSelect = successNs and CfgNewSelect or nil
                        local bestPart = nil
                ...
                -- 3. Kiểm tra MemePetFind (Tìm Meme Pet)
                if not activeObbyPart then
                    local memeFolder = nil
                    for _, child in ipairs(obbyFolder:GetChildren()) do
                        if string.find(child.Name, "MemePetFind") then
                            memeFolder = child:FindFirstChild("MemePetFindRuntimeZones")
                            if memeFolder then break end
                        end
                    end
                    
                    if memeFolder and #memeFolder:GetChildren() > 0 then
                        local bestPart = nil
                        local maxVotes = -1
                        local hudLines = { "Sự kiện: Tìm Meme Pet" }

                        -- Xóa vết ô đã chọn nếu ô đó không còn tồn tại trong folder
                        if lastSelectedMemePart and not lastSelectedMemePart:IsDescendantOf(memeFolder) then
                            lastSelectedMemePart = nil
                        end

                        local currentTargetOtherVotes = 0
                        
                        for _, cell in ipairs(memeFolder:GetChildren()) do
                            if cell:IsA("BasePart") then
                                -- Tính số NGUỜI CHƠI KHÁC đang đứng trên bục này (LOẠI TRỪ LOCALPLAYER TỰ ĐỨNG)
                                local playerCount = 0
                                for _, p in ipairs(Players:GetPlayers()) do
                                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                                        local dist = (p.Character.HumanoidRootPart.Position - cell.Position).Magnitude
                                        if dist < 15 then
                                ...
                                local labelName = cell.Name
                                local groupLabel = cell:FindFirstChild("TrickOrTreat") and cell.TrickOrTreat:FindFirstChild("Main") and cell.TrickOrTreat.Main:FindFirstChild("Group")
                                if groupLabel then
                                    labelName = labelName .. " (" .. groupLabel.Text .. ")"
                                end

                                if cell == lastSelectedMemePart then
                                    currentTargetOtherVotes = playerCount
                                end
                                
                                table.insert(hudLines, string.format("  • %s: Số người đứng = %d", labelName, playerCount))
                                if playerCount > maxVotes then
                                    maxVotes = playerCount
                                    bestPart = cell
                                end
                            end
                        end
                        
                        -- CHỐNG NHẢY TARGET (TARGET LOCKING HYSTERESIS):
                        -- Chỉ đổi ô khi có ô khác có số người chơi NGUYÊN NGUYÊN HƠN (maxVotes > currentTargetOtherVotes).
                        -- Nếu vote bằng nhau (hoặc không có người đứng), GIỮ NGUYÊN ô hiện tại để đứng yên an toàn!
                        if lastSelectedMemePart and lastSelectedMemePart:IsDescendantOf(memeFolder) then
                            if maxVotes > currentTargetOtherVotes then
                                lastSelectedMemePart = bestPart
                            end
                        else
                            lastSelectedMemePart = bestPart or memeFolder:GetChildren()[1]
                        end
                        
                        activeObbyPart = lastSelectedMemePart
                        if activeObbyPart then
                            table.insert(hudLines, "Đang chọn ổn định: " .. activeObbyPart.Name)
                        end
                        
                        LiveHud:Set({
                            Title = "📡 SỰ KIỆN TÌM MEME PET LIVE",
                            Content = table.concat(hudLines, "\n")
                        })
                    else
                        lastSelectedMemePart = nil
            ...
            -- Nếu phát hiện ô lựa chọn chính xác đang hoạt động, chỉ dịch chuyển nếu chưa đứng trên bục và đứng yên an toàn
            if activeObbyPart then
                EventStatus:Set(Utils.t("event_status_standing"))
                local targetPos = activeObbyPart.Position + Vector3.new(0, 3.5, 0)
                local dist = (hrp.Position - targetPos).Magnitude
                
                -- Chỉ set CFrame nếu đang ở xa bục (> 4 studs) để tránh nhân vật bị nhảy giật giật liên tục
                if dist > 4 then
                    hrp.CFrame = CFrame.new(targetPos)
                end
                task.wait(0.5) -- Đứng yên trên bục chờ kết quả
                continue
            end
            ...
            if #items > 0 then
                ...
                -- Tìm vật phẩm gần nhất mà chưa được claim
                local target = nil
                local cacheKey = nil
                for _, it in ipairs(items) do
                    local key = it.object:GetFullName() .. "_" .. tostring(it.pos)
                    if not claimedCache[key] then
                        target = it
                        cacheKey = key
                        break
                    end
                ...
                if target then
                    ...
                    local targetClaimPos = target.pos + Vector3.new(0, 3, 0)
                    local distItem = (hrp.Position - targetClaimPos).Magnitude
                    
                    -- Chỉ dịch chuyển nếu chưa ở sát vật phẩm
                    if distItem > 4 then
                        hrp.CFrame = CFrame.new(targetClaimPos)
                        task.wait(0.2) -- Chờ game stream/load vị trí vật lý
                    end
